...
> **TIP:** One commonly asked question is if some of the objects can be rendered without the Outline Renderer Feature applied. Outline image effect is global and is not designed to be selective to objects — it is applied per Forward Renderer (in URP). However, making something to use transparency pass is a working solution. In Flat Kit Stylized Surface there is such an option. If you are using non-Flat Kit shaders, please refer to their documentation to see whether it is possible - to enable transparency without affecting the look. In Flat Kit you gave an option of Render Event. It lets you choose when to render the lines. Take a look at [this chapter](#42-outline-image-effect) in our manual.  
